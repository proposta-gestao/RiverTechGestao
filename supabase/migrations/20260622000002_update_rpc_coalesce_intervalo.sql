-- ============================================================
-- Migration: Atualiza RPC para usar COALESCE(intervalo_servico, intervalo_agenda, 0)
-- Data: 2026-06-22
-- O motor lê o intervalo específico do serviço. Se NULL, cai para o intervalo
-- do horário de funcionamento. Se também NULL, usa 0.
-- ============================================================

CREATE OR REPLACE FUNCTION public.get_slots_disponiveis(
    p_empresa_id        UUID,
    p_profissional_id   UUID,
    p_servico_id        UUID,
    p_data              DATE
)
RETURNS TABLE (
    slot_inicio TIMESTAMPTZ,
    slot_fim    TIMESTAMPTZ
)
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
    v_duracao_min       INTEGER;
    v_servico_intervalo INTEGER; -- Intervalo específico do serviço (pode ser NULL)
    v_dia_semana        INTEGER;
    v_abertura          TIME;
    v_fechamento        TIME;
    v_agenda_intervalo  INTEGER; -- Intervalo da agenda/profissional
    v_intervalo         INTEGER; -- Intervalo efetivo (COALESCE dos dois acima)
    v_slot_inicio       TIMESTAMPTZ;
    v_slot_fim          TIMESTAMPTZ;
    v_hora_atual        TIME;
    v_fim_conflito      TIMESTAMPTZ;
    v_next_hora         TIME;
BEGIN
    -- Duração e intervalo do serviço
    -- Futuramente, quando o campo intervalo_min for adicionado por serviço,
    -- basta ler aqui. O COALESCE já garante retrocompatibilidade.
    SELECT duracao_min, intervalo_min
    INTO v_duracao_min, v_servico_intervalo
    FROM public.servicos
    WHERE id = p_servico_id;

    IF v_duracao_min IS NULL OR v_duracao_min <= 0 THEN RETURN; END IF;

    -- Dia da semana (0=domingo)
    v_dia_semana := EXTRACT(DOW FROM p_data);

    -- Horário de funcionamento (intervalo da agenda/profissional)
    SELECT hora_abertura, hora_fechamento, intervalo_min
    INTO v_abertura, v_fechamento, v_agenda_intervalo
    FROM public.horarios_funcionamento
    WHERE empresa_id = p_empresa_id
      AND ativo = true
      AND dia_semana = v_dia_semana
      AND (profissional_id = p_profissional_id OR profissional_id IS NULL)
    ORDER BY profissional_id NULLS LAST
    LIMIT 1;

    IF v_abertura IS NULL THEN RETURN; END IF;

    -- Intervalo efetivo:
    -- Prioridade 1: intervalo configurado no serviço
    -- Prioridade 2: intervalo da agenda/profissional
    -- Prioridade 3: 0 (sem intervalo)
    v_intervalo := COALESCE(v_servico_intervalo, v_agenda_intervalo, 0);

    v_hora_atual := v_abertura;

    WHILE (v_hora_atual + (v_duracao_min || ' minutes')::INTERVAL <= v_fechamento) LOOP
        v_slot_inicio := ((p_data::TEXT || ' ' || v_hora_atual::TEXT)::TIMESTAMP AT TIME ZONE 'America/Sao_Paulo');
        v_slot_fim    := v_slot_inicio + (v_duracao_min || ' minutes')::INTERVAL;

        -- Verificar conflito: pega o agendamento conflitante que termina mais tarde
        SELECT data_hora_fim INTO v_fim_conflito
        FROM public.agendamentos
        WHERE profissional_id = p_profissional_id
          AND status NOT IN ('cancelado', 'no_show')
          AND data_hora_inicio < v_slot_fim
          AND data_hora_fim > v_slot_inicio
        ORDER BY data_hora_fim DESC
        LIMIT 1;

        IF v_fim_conflito IS NOT NULL THEN
            -- HOUVE CONFLITO: pula para o fim do conflito + intervalo efetivo
            v_next_hora := (v_fim_conflito AT TIME ZONE 'America/Sao_Paulo')::TIME
                            + (v_intervalo || ' minutes')::INTERVAL;

            IF v_next_hora <= v_hora_atual THEN EXIT; END IF;
            v_hora_atual := v_next_hora;
        ELSE
            -- LIVRE: emite o slot
            slot_inicio := v_slot_inicio;
            slot_fim    := v_slot_fim;
            RETURN NEXT;

            -- Avança pelo tamanho do serviço + intervalo efetivo
            v_next_hora := v_hora_atual + ((v_duracao_min + v_intervalo) || ' minutes')::INTERVAL;

            IF v_next_hora <= v_hora_atual THEN EXIT; END IF;
            v_hora_atual := v_next_hora;
        END IF;

    END LOOP;
END;
$$;
