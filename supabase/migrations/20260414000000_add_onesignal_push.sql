-- Habilita a extensÃ£o pg_net (PadrÃ£o no Supabase para requisiÃ§Ãµes HTTP REST dentro do banco)
...
-- 2. Cria a funÃ§Ã£o de trigger para disparar o Push Notification via requisiÃ§Ã£o POST
...
DECLARE
    ...
    player_ids JSONB;
BEGIN
    -- Busca todos os onesignal_ids cadastrados (que nÃ£o sÃ£o nulos) para notificar TODOS os atendentes
    SELECT jsonb_agg(onesignal_id) INTO player_ids
    FROM public.atendentes
    WHERE onesignal_id IS NOT NULL AND onesignal_id != '';

    -- Se nÃ£o tem ninguÃ©m com notificaÃ§Ãµes ativas, sai da funÃ§Ã£o silenciosamente
    IF player_ids IS NULL OR jsonb_array_length(player_ids) = 0 THEN
        RETURN NEW;
    END IF;

    -- ConstrÃ³i o corpo da requisiÃ§Ã£o para a API da OneSignal
    json_body := jsonb_build_object(
        'app_id', 'c7223246-03a4-4fff-b9f3-f6217b183917',
        'include_player_ids', player_ids,
        'headings', jsonb_build_object('en', 'ðŸ”” Novo Pedido # ' || left(NEW.id::text, 8)),
    ...
    -- 3. Dispara a requisiÃ§Ã£o HTTP POST assÃ­ncrona (nÃ£o atrasa o insert da tabela)
...
-- 4. Associa a Trigger Ã  tabela de pedidos (somente no INSERT e quando for pedido novo = pendente)
